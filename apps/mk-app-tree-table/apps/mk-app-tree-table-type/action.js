...
class action {
    ...
    load = async () => {
        const response = await this.webapi.goodsType.findById(this.component.props.typeId)
    ...
    save = async () => {
        ...
        if (form.id) {
            const response = await this.webapi.goodsType.update(form)
            this.metaAction.toast('success', '修改成功')
            return response
        }
        else {
            const response = await this.webapi.goodsType.create(form)
